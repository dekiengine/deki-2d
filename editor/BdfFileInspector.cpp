...
void BdfFileInspector::OnInspectorGUI(const std::string& assetPath, const std::string& assetGuid)
{
    ...
    {
        char buf[256];
        std::snprintf(buf, sizeof(buf), "BDF Font: %s", path.filename().string().c_str());
        ui.Text(buf);
    }
    if (!assetGuid.empty())
    {
    ...
    {
        ui.TextColored(1.0f, 0.3f, 0.3f, 1.0f, "No glyphs found (codepoints 0-255)");
        return;
    }
    ...
    {
        char buf[64];
        std::snprintf(buf, sizeof(buf), "%d selected", (int)m_SelectedChars.size());
        ui.Text(buf);
    }
    ui.SameLine();
    if (ui.Button("Clear"))
    ...
    {
        char buf[64];
        std::snprintf(buf, sizeof(buf), "%d glyphs shown", (int)filtered.size());
        ui.Text(buf);
    }
    ...
    while (ui.ListClipperStep(&clipStart, &clipEnd))
    {
        for (int row = clipStart; row < clipEnd; row++)
        {
            ...
            if (isSelected)
            {
                ui.DrawRectFilled(rowStartX, rowStartY, rowEndX, rowEndY, DekiEditor::EditorUI::Rgba(86, 158, 255, 80));
                ui.DrawRect(rowStartX, rowStartY, rowEndX, rowEndY, DekiEditor::EditorUI::Rgba(86, 158, 255, 200));
            }
